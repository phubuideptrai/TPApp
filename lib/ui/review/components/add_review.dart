                        ...
                        onPressed: () async {
                          if (fileImageArray.length < 1 && comment == "") {
                            Fluttertoast.showToast(
                                msg:
                                    'Please leave a comment or image before leaving a review.',
                            ...
                            bool result =
                                await reviewServices.addReviews(newReviews);
                            if (result == false) {
                              EasyLoading.showError(
                                  'Some errors happened when adding this review');
                              Future.delayed(const Duration(milliseconds: 1000),
                                  () {
                                EasyLoading.dismiss();
                              });
                            } else {
                              bool result2 = await orderServices
                                  .updateOrderStatus(widget.orderID, 4);
                              if (result2 == false) {
                                EasyLoading.showError(
                                    'Some errors happened when adding this review');
                                Future.delayed(
                                ...
                                EasyLoading.showSuccess(
                                    'Added this review successfully');
                                Future.delayed(
                                    const Duration(milliseconds: 1000), () {
                                  EasyLoading.dismiss();
                                });
