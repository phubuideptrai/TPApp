...
//Insertion Sort
  ...
  Future<List<MProduct>> getSimilarityProductsByCBR(
    ...
    http.Response response = await http.post(
      Uri.parse('${Variable.url}/api/getSimilarProductsBasedUserByCBR'),
  ...
  Future<List<MProduct>> getSimilarityProductsByCFR(
    ...
    http.Response response = await http.post(
      Uri.parse('${Variable.url}/api/getSimilarProductsByCFR'),
      headers: {"Content-Type": "application/json"},
      body: json.encode(Data),
    );
    var data = await json.decode(response.body);
  ...
  Future<List<MProduct>> getSimilarityProductsBySelectedProduct(
    ...
    http.Response response = await http.post(
      Uri.parse('${Variable.url}/api/getSimilarProductBySelectedProduct'),
      headers: {"Content-Type": "application/json"},
      body: json.encode(Data),
    );
    print('similarity: ${response.body}');
    var data = await json.decode(response.body);
    for (int i = 0; i < data['result'].length; i++) {
      int id = data['result'][i]['id'];
