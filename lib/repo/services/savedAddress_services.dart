...
class SavedAddressServices {
  ...
  Future<MSavedAddress> addSavedAddress(
    ...
    MSavedAddress temp =
        new MSavedAddress("", "", "", "", "", false, false, 0.0, 0.0);
    try {
      String addressID = "";

      await refSavedAddress.doc().set({
        'addressID': "",
        'userID': userID,
      ...
      if (addressID != "") {
        ...
        if (result == true) {
          MSavedAddress result_SavedAddress =
              await getSavedAddressByAddressID(addressID);
  ...
  Future<MSavedAddress> getSavedAddressByStore() async => await refSavedAddress
  ...
  void copySavedAddress(
      MSavedAddress savedAddress1, MSavedAddress savedAddress2) {
  ...
  Future<bool> updateDefaultSavedAddress(
  ...
  Future<bool> updateSavedAddress(MSavedAddress address) async {
    try {
      if (address.getFullAddressName() != "")
        await refSavedAddress
            .doc(address.getID())
            .update({'fullAddressName': address.getFullAddressName()});
      if (address.getFullUserName() != "")
        await refSavedAddress
            .doc(address.getID())
            .update({'fullUserName': address.getFullUserName()});
      if (address.getPhone() != "")
        await refSavedAddress
            .doc(address.getID())
            .update({'userPhone': address.getPhone()});
      if (address.getLatitude() > 0)
        await refSavedAddress
            .doc(address.getID())
            .update({'latitude': address.getLatitude()});
      if (address.getLongitude() > 0)
        await refSavedAddress
            .doc(address.getID())
            .update({'longitude': address.getLongitude()});
